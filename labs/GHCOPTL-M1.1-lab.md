...
1. En el panel de chat, en **modo agente**, pídeselo:
   ...
   **→ Qué esperar.** GitHub Copilot usa `gh` en el terminal: crea el repositorio en tu cuenta —con algo parecido a `gh repo create <nombre> --public --source=. --push`—, lo enlaza como `origin` y sube tu código. Te pedirá **confirmación** antes de ejecutar comandos que tocan tu cuenta —crear un repositorio no es cualquier cosa—; ahí decides tú.
   ...
   > **🔎 El gancho de lo que viene.** Fíjate en lo que acaba de pasar: le has pedido en lenguaje natural algo que toca un servicio externo al editor, y lo ha hecho por ti. Hoy, con `gh` como intermediario. En el último capítulo darás el salto —con **MCP** conectado, GitHub Copilot hablará con GitHub directamente, sin comando de por medio—. Es la última capa del mapa; hoy solo la rozas.
